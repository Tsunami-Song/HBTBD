...
def load_Elliptic_data(time_step):
    prefix = 'F:/AML/MyWork/Data_process/{}_data/'.format(time_step)
    in_file = open(prefix + '{}_m1.adjlist'.format(time_step), 'r')
    adjlist00 = [line.strip() for line in in_file]
    in_file.close()
    in_file = open(prefix + '{}_m2.adjlist'.format(time_step), 'r')
    adjlist01 = [line.strip() for line in in_file]
    in_file.close()
    in_file = open(prefix + '{}_m3.adjlist'.format(time_step), 'r')
    adjlist02 = [line.strip() for line in in_file]
    in_file.close()

    in_file = open(prefix + '{}_idx00.pickle'.format(time_step), 'rb')
    idx00 = pickle.load(in_file)
    in_file.close()
    in_file = open(prefix + '{}_idx01.pickle'.format(time_step), 'rb')
    idx01 = pickle.load(in_file)
    in_file.close()
    in_file = open(prefix + '{}_idx02.pickle'.format(time_step), 'rb')
    idx02 = pickle.load(in_file)
    in_file.close()

    #features_0 = scipy.sparse.load_npz(prefix + '/features_0.npz').toarray()
    #features_1 = scipy.sparse.load_npz(prefix + '/features_1.npz').toarray()
    features_0 = np.load(prefix + '/{}_features0.npy'.format(time_step))
    features_1 = np.load(prefix + '/{}_features1.npy'.format(time_step))
    features_2 = np.load(prefix + '/{}_features2.npy'.format(time_step))
    features_3 = np.load(prefix + '/{}_features3.npy'.format(time_step))

    adjM = scipy.sparse.load_npz(prefix + '/{}_sparse_matrix.npz'.format(time_step))
    type_mask = np.load(prefix + '/{}_node_types.npy'.format(time_step))
    labels = np.load(prefix + '/{}_labels.npy'.format(time_step))

    train_val_test_idx = np.load(prefix + '/{}_train_val_test_idx.npz'.format(time_step))
