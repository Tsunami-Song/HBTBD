...
tx_features = ["txfea_" + str(i) for i in range(2, 95)]
agg_features = ["aggfea_" + str(i) for i in range(1, 73)]
lf = tx_features
...
def svm_test(X, y, modelname,test_sizes=(0.2, 0.4, 0.6, 0.8), repeat=1):
    random_states = 0
    ...
    for test_size in test_sizes:
        ...
        X_train, X_test, y_train, y_test = train_test_split(
            X, y, test_size=test_size, shuffle=True, random_state=random_states)
        ...
        print("TestSize:{}%".format(test_size * 100))
        print('---------------------------------------------------------------------------------------------')
        print('|  Method  |  Precision  |   Recall   |    F1    | MicroAVG F1 | Illicit / Licit Sample num |')
        print(
            "|  MAGNN   |    {:.3f}    |    {:.3f}   |   {:.3f}  |    {:.3f}    |       {:5} / {:5}        |".format(
                Precision[0], recall[0], f1score[0], micro_f1, num[0], num[1]))
        print('---------------------------------------------------------------------------------------------')
...
def feature_df_plot(features_df,modelname):
    #plt.figure(figsize=(60,10),dpi=200)
    sns.set(rc={"figure.figsize": (20, 5),"figure.dpi":300})
    ax = sns.barplot(x = features_df['Features'][:20], y = features_df['Importance'][:20])
    color_list = list()
    for i in features_df['Features'][:20]:
        if "EMB" in i:
            color_list.append("Red")
        else:
            color_list.append("Blue")
    for bar_container in ax.containers:
        for bar,group_cmap in zip(bar_container, color_list):

            bar.set_color(group_cmap)
    ax.legend(['AF_features', 'EMB_features'],loc='upper right', frameon=False)
    plt.ylabel('Importance')
    sns.despine(bottom=True)
    plt.title(modelname)
    plt.savefig("{}.jpg".format(modelname))
    #plt.show()
    return plt

def random_forest_test(X, y, modelname,test_sizes=(0.2, 0.4, 0.6, 0.8), repeat=1):
    ...
    for test_size in test_sizes:
        ...
        for i in range(repeat):
            X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=test_size, shuffle=False, random_state=random_states)
            rfc = RandomForestClassifier(random_state=random_states,n_estimators=100,max_features=100,n_jobs=-1)#,class_weight='balanced')
            rfc = rfc.fit(X_train, y_train)
            y_pred = rfc.predict(X_test)
            if (X_train.shape[1] ==677 and test_size ==0.8):
            ...
            elif(X_train.shape[1] ==605 and test_size ==0.8):
                features = ["EMB_{}".format(i) for i in range (512)] +lf
                feature_importances = rfc.feature_importances_
                features_df = pd.DataFrame({'Features': features, 'Importance': feature_importances})
                features_df.sort_values('Importance', inplace=True, ascending=False)
                plt = feature_df_plot(features_df,modelname)
            ...
            print('---------------------------------------------------------------------------------------------')
            print('|  Method  |  Precision  |   Recall   |    F1    | MicroAVG F1 | Illicit / Licit Sample num |')
            print("|  MAGNN   |    {:.3f}    |    {:.3f}   |   {:.3f}  |    {:.3f}    |       {:5} / {:5}        |".format(Precision[0],recall[0],f1score[0],micro_f1,num[0],num[1]))
            print('---------------------------------------------------------------------------------------------')
            macro_f1_list.append(macro_f1)
            micro_f1_list.append(micro_f1)
        result_macro_f1_list.append((np.mean(macro_f1_list), np.std(macro_f1_list)))
...
def evaluate_results_nc(embeddings, labels, num_classes,modelname):
    '''print('SVM test')
    ...
    svm_macro_f1_list, svm_micro_f1_list = svm_test(embeddings, labels,modelname)
    #random_forest_macro_f1_list, random_forest_micro_f1_list = random_forest_test(embeddings, labels,modelname)

    '''print('Ramdom Forest test')
    print('Macro-F1: ' + ', '.join(['{:.6f}~{:.6f} ({:.1f})'.format(macro_f1_mean, macro_f1_std, train_size) for
                                    (macro_f1_mean, macro_f1_std), train_size in
                                    zip(svm_macro_f1_list, [0.8, 0.6, 0.4, 0.2])]))
    print('Micro-F1: ' + ', '.join(['{:.6f}~{:.6f} ({:.1f})'.format(micro_f1_mean, micro_f1_std, train_size) for
