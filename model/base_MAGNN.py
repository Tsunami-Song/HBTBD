...
class MAGNN_metapath_specific(nn.Module):
    ...
    def forward(self, inputs):
        ...
        if self.rnn_type == 'gru':
            ...
            hidden = hidden.unsqueeze(dim=0)
        elif self.rnn_type == 'TransE0' or self.rnn_type == 'TransE1' or self.rnn_type == 'TransE2' or self.rnn_type == 'TransE3':
            r_vec = self.r_vec
            if (self.rnn_type == 'TransE0' or self.rnn_type == 'TransE2'):
                r_vec = torch.stack((r_vec, -r_vec), dim=1)
                r_vec = r_vec.reshape(self.r_vec.shape[0] * 2, self.r_vec.shape[1])  # etypes x out_dim
            edata = F.normalize(edata, p=2, dim=2)  #L2 norm
            ...
            elif (self.rnn_type == 'TransE2' or self.rnn_type == 'TransE1'):
                for i in range(edata.shape[1] - 1):
                    # consider None edge (symmetric relation)
                    temp_etypes = [etype for etype in self.etypes[i:] if etype is not None]
                    edata[:, i*2] = edata[:, i*2] + r_vec[temp_etypes].sum(dim=0)
            elif (self.rnn_type == 'TransE3'):
                for i in range(edata.shape[1] - 1):
                    # consider None edge (symmetric relation)
                    temp_etypes = [etype for etype in self.etypes[i:] if etype is not None]
                    edata[:, abs(i-1)] = edata[:, abs(i-1)] + r_vec[temp_etypes].sum(dim=0)
            hidden = torch.mean(edata, dim=1)
            hidden = torch.cat([hidden] * self.num_heads, dim=1)
            hidden = hidden.unsqueeze(dim=0)
